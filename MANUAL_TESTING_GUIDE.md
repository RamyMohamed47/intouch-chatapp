...
| RT-04 | Last seen               | Stay offline past the grace period and reload another member's roster.                                                                | Status is offline and the UI shows relative last-seen time from the latest final disconnect.                                                      |
| RT-05 | Subscription snapshot   | Disconnect and reconnect while another member changes presence, then reopen the organization.                                         | The roster reconciles after organization subscription and shows current presence even if an event was missed while disconnected.                  |
| RT-06 | Typing start/stop       | Type non-whitespace content, pause, clear, blur, send, and navigate away.                                                             | An animated, politely announced indicator appears above the composer and stops on clear/blur/send/navigation.                                     |
| RT-07 | Typing expiration       | Interrupt the typing client without sending an explicit stop.                                                                         | Server state expires after roughly 5 seconds; the receiving client defensively removes stale state by 7 seconds.                                  |
| RT-08 | Typing isolation        | Type in one conversation while another account views a different conversation.                                                        | Indicator appears only to authorized members in the same conversation and not on the typing user's other sockets.                                 |
